...
# Knowledge Vault — AI agent 驱动的跨项目知识系统

> 一个专为 Claude Code agent 设计的 Obsidian vault 模板。
> 把多个代码项目的 docs/ 镜像到只读区，然后由 agent 在可写区提炼跨项目模式。
> 带同步脚本 + 一份行为契约（[`AGENT-GUIDE.md`](AGENT-GUIDE.md)）给 agent。
> 只有方法论 —— 不含任何真实项目文档；clone 下来用你自己的项目填充。
...
- **[`CLAUDE.md`](CLAUDE.md) + [`AGENT-GUIDE.md`](AGENT-GUIDE.md)** — 给 Claude Code 的行为契约：什么能动、什么不能动、遇到新同步怎么 diff、跨项目模式什么时候抽出来、笔记用什么格式写。
...
```
knowledge-vault/
├── README.md / README.zh-CN.md     ← 英文主 / 中文变体
├── LICENSE                         ← MIT
├── CLAUDE.md / CLAUDE.en.md        ← agent 入口（中文主 / 英文变体）
├── AGENT-GUIDE.md / AGENT-GUIDE.en.md  ← agent 手册（中文主 / 英文变体）
├── .gitignore
...
> **关于 agent 文档的语言**：`CLAUDE.md` 和 `AGENT-GUIDE.md` 默认是**中文**，因为 Claude Code 启动时自动加载它们、本模板作者的工作流也是中文。英文镜像在 `CLAUDE.en.md` 和 `AGENT-GUIDE.en.md`。如果你想用英文工作流，把 `.en.md` 那份改名顶替掉原 `.md` 即可。
...
- 跨 roundtrip / 跨系统 / 跨项目的引用用 **name 不用 UUID**（人类可读 + 跨序列化稳定）
- 冲突或歧义时 agent 必须**停下来问用户**，不要武断合并

除此之外都可以按自己的需求改：

- `AGENT-GUIDE.md` 里的 5 步工作流（加/减步骤）
- 笔记模板的 frontmatter 字段
- `synthesized/` 和 `cross-project/` 的子分类（`patterns/decisions/gotchas/` 只是一个起点）
- 同步脚本（可以用别的工具代替 rsync/robocopy）
